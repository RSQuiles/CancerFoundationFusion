...
def run(
    ...
    # Extract paired cell lines (cell lines with both SC and bulk)
    paired_cls = sorted(
        set(sc_adata.obs[cell_line_col]) &
        set(bulk_adata.obs[cell_line_col])
    ) if sc_adata is not None and bulk_adata is not None else []
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--pca", action="store_true", help="Whether to use PCA as embedder")
    return p
